...
由外层 AI 监督时，`/loopsail:run-once` 每次先由 Coordinator `prepare-step` 建立 attempt lease，再前台串行调用一个可见的 `loopsail:worker` 子 Agent，最后无论 Agent 成功、阻塞或意外结束都执行 `finalize-step`。Worker 没有 LoopSail 自设的秒级超时或预算，运行由 Claude Code 和用户控制；不要创建后台 Worker 或并发 Agent。

每个底层 action 的 stdout 都是唯一 `command-envelope` v2 JSON，真实退出码与 envelope 一致；步骤报告同时写入 `.loopsail/runs/<list-id>/last-step.json`。退出码 `0` 表示成功或完成，`2` 表示阻塞/预期错误，`3` 表示本步有进展且应继续，`4` 表示当前无可运行任务。只能在完整的 prepare/Agent/finalize 单元之间修改 `TASKS.json`。
...
整轮托管必须由用户显式调用 `/loopsail:run-all`，并由 Supervisor 顺序循环前台子 Agent。不要让完整运行与单步运行并发；项目锁、attempt lease 和 agent ID 会 fail-closed。

不要手工修改 `.loopsail/input/`、`.loopsail/output/`、`.loopsail/runs/`、`.loopsail/logs/` 或锁文件。完成的任务列表保持不可变；后续工作使用新的 `list_id`。LoopSail 2.0 不迁移 v1 状态。
